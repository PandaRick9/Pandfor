...
CREATE TABLE ResumeSkill (
                             resume_id INT REFERENCES Resume(resume_id),   -- Привязка к резюме
                             skill_id INT REFERENCES Skill(skill_id),      -- Привязка к навыку
...
CREATE TABLE VacancySkill (
                              vacancy_id INT REFERENCES Vacancy(vacancy_id),-- Привязка к вакансии
                              skill_id INT REFERENCES Skill(skill_id),      -- Привязка к навыку
                              required_level VARCHAR(50),                   -- Требуемый уровень владения
